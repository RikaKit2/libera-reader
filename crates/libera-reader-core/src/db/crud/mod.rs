...
pub(crate) fn get_primary<T: ToInput>(key: impl ToKey, db: &Database) -> Option<T> {
...
pub(crate) fn insert_batch<T: ToInput>(data: Vec<T>, db: &Database) {
  if data.len() > 0 {
...
pub(crate) fn update_table<FN, Table: ToInput + Clone>(db: &DB, table: Option<Table>, changing_fn: FN) where FN: FnOnce(&mut Table) {
  ...
  update::<Table>(old_table, new_table, db).unwrap();
}
