    ...
    if self.target_ext.read().await.contains(&ext) {
      ...
      match crud::get_book_data(book_hash.clone(), &self.client).await {
        None => {
          ...
          let book_data = crud::create_book_data(book_hash.clone(), file_size, ext, &self.client).await;
          crud::create_book_item(path_to_book.clone(), book_data.id, path_to_dir,
                                 dir_name, book_name, &self.client).await;
          self.not_cached_books.write().await.push_front(NotCachedBook { book_hash, path_to_book });
        }
        Some(book_data) => {
  ...
  pub async fn rename_book_data(&mut self, old_path: &PathBuf, new_path: &PathBuf) {
    ...
    match crud::get_book_item_by_path(old_path_to_book, &self.client).await {
      ...
      Some(old_book) => {
        ...
        if dir_equals == false {
          crud::change_path_and_dir(new_path_to_book, old_book.path_to_book, new_path_to_dir, new_dir_name, &self.client).await;
          return;
        } else if book_name_equals == false {
          crud::change_path_and_book_name(new_path_to_book, old_book.path_to_book, new_book_name, &self.client).await;
          return;
        } else if ext_equals == false {
          crud::change_path_and_ext(new_path_to_book, old_book.path_to_book, new_ext, &self.client).await;
        }
      }
    }
  }
