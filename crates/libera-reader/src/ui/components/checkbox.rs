...
use gpui::{
  AnyElement, App, Div, ElementId, InteractiveElement, IntoElement, ParentElement, RenderOnce, SharedString, StatefulInteractiveElement, Styled, Window, div,
  prelude::FluentBuilder as _, relative, rems, rgb, svg,
};
...
impl Checkbox {
  pub fn new(id: impl Into<ElementId>) -> Self {
    Self { id: id.into(), base: div(), label: None, children: Vec::new(), checked: false, disabled: false, size: Size::default(), on_click: None }
  }
      ...
      self
        ...
        .child(
          div()
            ...
            .flex_shrink_0()
            .border_1()
            .when(!self.checked, |this| this.border_color(rgb(adjust_brightness(theme.base_color_content, 0.4))))
            ...
            div()
              ...
              .map(|this| {
                if let Some(label) = self.label {
                  this.child(div().size_full().text_color(rgb(theme.base_color_content)).line_height(relative(1.)).child(label))
                } else {
                  this
                }
              })
        ...
        .when(self.disabled, |this| this.text_color(rgb(adjust_brightness(theme.base_color_content, 0.5))))
