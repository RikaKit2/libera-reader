...
use gpui::{
  App, AppContext, Context, Entity, FontWeight, InteractiveElement, IntoElement, MouseButton, MouseDownEvent, ParentElement, Render, Styled, Window, div, rgb,
};
  ...
  fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    div().bg(rgb(theme.base_100)).w_full().h_full().p_6().flex().flex_col().justify_between().text_color(rgb(theme.base_color_content)).children([
      ...
      div().w_full().flex().justify_end().children([div()
        .px_2()
        .text_color(rgb(theme.info_content_color))
        .rounded_sm()
        .font_weight(FontWeight::BOLD)
