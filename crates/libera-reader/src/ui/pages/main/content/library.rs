...
/// Resolve the thumbnail PNG path for every book key, preserving order.
/// `None` entries (no book record / no PNG on disk) keep index alignment with
/// the keys list so the grid can render a placeholder for them.
...
impl Library {
  pub fn new(window: &mut Window, cx: &mut Context<Self>, books_state: Entity<BooksState>) -> Self {
    let top_bar = cx.new(|cx| TopBar::new(window, cx, books_state.clone(), TargetList::Library));
    ...
    let paths = resolve_thumbnail_paths(&books_state.read(cx).library_keys, &db, &thumbnails_dir);
    ...
    let _subscriptions = vec![cx.observe(&books_state, move |this, state, cx| {
      let thumbnails_dir = Ctx::global(cx).app_dirs.read().thumbnails_dir.clone();
      let db = Ctx::global(cx).db.clone();
      let paths = resolve_thumbnail_paths(&state.read(cx).library_keys, &db, &thumbnails_dir);
